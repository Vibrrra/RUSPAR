...
pub fn update_source_tree(source_trees: &mut SourceTrees, sources: Vec<Source>, room: &Room, ism_order: usize) {
    ...
    //         for i in N_IS_INDEX_RANGES[order].0+n*(offset+1) .. N_IS_INDEX_RANGES[order].1+n*(offset+1) {
    //             for boundary in Reflected::VALUES {
    //                 let current_source = source_tree.arena.get(source_tree.node_list[i]).unwrap().get();
    //                 if current_source.reflector != boundary  {
    //                     let mut_node = source_tree.arena.get_mut(source_tree.node_list[i]).unwrap().get_mut(); 
    //                     mut_node.position = calc_ism_position(current_source, room, &boundary);
