...
use crate::image_source_method::{
    self, update_source_tree, update_source_tree_from_roots, Listener, Room, Source, SourceTrees,
    SourceType, SphericalCoordinates,
};
...
pub fn update_scene(scene: &Scene_data, source_trees: &mut SourceTrees<Source>) {
    ...
    scene
        ...
        .transforms
        .iter()
        .zip(source_trees.roots.iter())
        .zip(source_trees.arenas.iter_mut())
        .for_each(|((transform, node_id), arena)| {
            ...
            (
                _,
                src.source_listener_orientation.azimuth,
                src.source_listener_orientation.elevation,
            ) = calculate_azimuth_and_elevation_with_rotation(transform, &listener_transform);
        });
    let room: Room = Room {
...
pub fn calculate_azimuth_and_elevation_with_rotation(
    a: &Transform,
    b: &Transform,
) -> (f32, f32, f32) {
    //Rotation<f32,3> {
