...
use crate::{
    audioSceneHandlerData::{Scene_data, Sources, Transform},
    audio_module::ISMDelayLine,
    config::IMAGE_SOURCE_METHOD_ORDER,
    convolver::Spatializer,
    delaylines::DelayLine,
    image_source_method::{
        calc_ism_position, create_ism, Listener, Reflected, Room, Source, SourceType,
        SphericalCoordinates, N_IS_INDEX_RANGES,
    },
};
...
impl ISMDelayLines {
    pub fn new(
        ...
        let max_distance: f32 =
            (room.dimension.x.powi(2) + room.dimension.y.powi(2) + room.dimension.z.powi(2)).sqrt();
        let max_length: usize = (max_distance / c * sample_rate).ceil() as usize;
        let mut v = Vec::new();
        for i in 0..sources.sources.len() {
            let mut v2 = Vec::new();
            let ism_delayline: ISMDelayLine =
                ISMDelayLine::new(max_length, buffer_size, spatializer.clone());
    ...
    pub fn init_from_scene(scene: Scene_data, ism_order: usize) -> Self {
        ...
        let listener: Listener = Listener {
            position: Vector3::new(
                listener_transform.position.x,
                listener_transform.position.y,
                listener_transform.position.z,
            ),
            orientation: get_quaternion(&listener_transform),
        };
        let mut scene_iter = scene.sources.transforms.iter();
        let room: Room = Room {
            dimension: Vector3::new(scene.room.width, scene.room.height, scene.room.length),
        };
        ...
        while let Some(scene_src) = scene_iter.next() {
            ...
            src.set_pos(Vector3::new(
                scene_src.position.x,
                scene_src.position.y,
                scene_src.position.z,
            ));
            ...
            for parent_idx in 0..ISM_INDEX_RANGES.len() {
                ...
                for boundary in Reflected::VALUES {
                    ...
                    if boundary != parent_src.get_reflector() {
                        ...
                        update_lst_src_orientation_from_transform(
                            &listener_transform,
                            scene_src,
                            &mut src,
                        );
                        update_src_lst_orientation_from_transform(
                            scene_src,
                            &listener_transform,
                            &mut src,
                        );
        ...
        for mut sources in &mut self.sources {
            ...
            for parent_idx in 0..ISM_INDEX_RANGES.len() {
                ...
                for i in idx_start..idx_stop {
                    ...
                    src.set_dist(calc_distance(&src.get_pos(), &listener.get_pos()));
                    src.set_remaining_dist(src.get_dist() - parent_dist);
                    update_lst_src_orientation_from_quat(
                        listener.orientation,
                        &listener.position,
                        &src.get_pos(),
                        src,
                    );
                    update_src_lst_orientation_from_quat(
                        src.get_orientation(),
                        &src.get_pos(),
                        &listener.position,
                        src,
...
fn update_lst_src_orientation_from_transform(
...
fn update_src_lst_orientation_from_transform(
    transform_a: &Transform,
    transform_b: &Transform,
    src: &mut Source,
) {
...
fn update_lst_src_orientation_from_quat(
    a_quat: Quaternion<f32>,
    a_pos: &Vector3<f32>,
    b_pos: &Vector3<f32>,
    src: &mut Source,
) {
...
fn update_src_lst_orientation_from_quat(
    a_quat: Quaternion<f32>,
    a_pos: &Vector3<f32>,
    b_pos: &Vector3<f32>,
    src: &mut Source,
...
// Utility Functions
pub fn calculate_azimuth_and_elevation_with_from_transform(
    a: &Transform,
    b: &Transform,
) -> (f32, f32, f32) {
    //Rotation<f32,3> {
...
pub fn calculate_azimuth_and_elevation_with_from_orientation(
    a_quat: Quaternion<f32>,
    a_pos: &Vector3<f32>,
    b_pos: &Vector3<f32>,
) -> (f32, f32, f32) {
    ...
    let relative_position: Vec<f32> = vec![b_pos.x - a_pos.x, b_pos.y - a_pos.y, b_pos.z - a_pos.z];
...
pub fn calc_lst_src_orientation(
    listener: &Listener,
    src_position: Vector3<f32>,
) -> SphericalCoordinates {
    let relative_position: Vec<f32> = vec![
        src_position.x - listener.position.x,
...
pub fn calc_src_lst_orientation<T: SourceType<Source>>(
    source: &T,
    lst_position: Vector3<f32>,
) -> SphericalCoordinates {
    let position = source.get_pos();
    let relative_position: Vec<f32> = vec![
        lst_position.x - position.x,
