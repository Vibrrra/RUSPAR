use crate::filter::{BinauralFilter, BinauralFilterType, FFTManager, FilterStorage};
...
impl Spatializer {
    ...
    pub fn new(blocksize: usize, fft_manager: FFTManager, filter_storage: &FilterStorage) -> Self {
        ...
        let n_segments_ds: usize =
            filter_storage.get_n_stereo_segments(BinauralFilterType::DirectSound);
    ...
    pub fn process(
        ...
        self.fft_manager
            .transform_to_f_with_scratch(&mut self.input_buf, &mut self.input_f[self.index]);
        ...
        if true {
            // deactivate active conv when false (debugging)
            self.temp_buf_l
                .iter_mut()
                .zip(
        ...
        self.fft_manager
            .transform_to_t_with_scratch(&mut self.temp_buf_l, &mut self.temp_output_buf_l);
        self.fft_manager
            .transform_to_t_with_scratch(&mut self.temp_buf_r, &mut self.temp_output_buf_r);
        ...
        output.chunks_mut(2).enumerate().for_each(|(i, s)| {
            ...
            s[0] += (self.temp_output_buf_l[self.n_points + i] * self.fade_in[i]
                + self.temp_output_prev_buf_l[self.n_points + i] * self.fade_out[i])
                * gain;
            s[1] += (self.temp_output_buf_r[self.n_points + i] * self.fade_in[i]
                + self.temp_output_prev_buf_r[self.n_points + i] * self.fade_out[i])
                * gain;
