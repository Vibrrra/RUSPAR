...
pub const N_IS_INDEX_RANGES: [(usize, usize); 4] = [(0, 1), (1, 7), (7, 37), (37, 187)];
...
impl SourceType<Source> for Source {
    ...
    fn create_ism(
        ...
        r: &Room,
        b: &Reflected,
        _may_have_spatializer: Option<Spatializer>,
    ) -> Source {
...
pub trait SourceType<T: Clone> {
    ...
    fn create_ism(
        ...
        room: &Room,
        b: &Reflected,
        may_have_spatializer: Option<Spatializer>,
    ) -> T;
    fn create_default(may_have_spatializer: Option<Spatializer>) -> Self;
    fn get_spatializer(&self) -> Option<Spatializer>;
    fn set_spatializer(&mut self, Spatializer: Spatializer);
...
//
pub fn from_source_tree<U>(
    source_tree: SourceTrees<U>,
    block_size: usize,
) -> SourceTrees<ISMLine<U>>
where
    U: SourceType<Source> + Clone,
{
    ...
    for (src_arena, src_node_list) in source_tree.arenas.iter().zip(source_tree.node_lists.iter()) {
        ...
        let src = src_arena
            .get(*src_node_list_iter.next().unwrap())
            .unwrap()
            .get();
    ...
    T: Clone + SourceType<T>,
{
            ...
            for i in N_IS_INDEX_RANGES[order].0..N_IS_INDEX_RANGES[order].1 {
                for boundary in Reflected::VALUES {
                    ...
                    if current_source.get_reflector() != boundary {
                        let new_node = arena.new_node(T::create_ism(
                            current_source,
                            room,
                            &boundary,
                            current_source.get_spatializer().clone(),
                        ));
    ...
    let source_tree = SourceTrees {
        arenas: source_tree_vec,
        node_lists,
        roots,
    };
    source_tree
...
pub fn update_source_tree(
    source_trees: &mut SourceTrees<Source>,
    sources: Vec<Source>,
    room: &Room,
    listener: &Listener,
) {
    ...
    for (n, new_source) in sources.iter().enumerate() {
        ...
        for i in 0..source_trees.arenas[n].count() {
            ...
            let parent_node: Option<NodeId> = source_trees.arenas[n]
                .get(source_trees.node_lists[n][i])
                .unwrap()
                .parent();
            match parent_node {
                Some(pn) => {
                    let parent_dist = source_trees.arenas[n].get(pn).unwrap().get().get_dist();
                    let parent_position: nalgebra::Matrix<
                        f32,
                        nalgebra::Const<3>,
                        nalgebra::Const<1>,
                    ...
                    //     parent_src.get_pos();
                    let current_node = source_trees.arenas[n]
                        .get_mut(source_trees.node_lists[n][i])
                        .unwrap()
                        .get_mut();
                    *current_node.get_pos() =
                        *calc_ism_position(&parent_position, room, &current_node.get_reflector());
                    current_node
                        .set_dist(calc_distance(&current_node.get_pos(), &listener.get_pos()));
                    current_node.set_remaining_dist(current_node.get_dist() - parent_dist);
                }
                None => {
                    let data = source_trees.arenas[n]
                        .get_mut(source_trees.node_lists[n][i])
                        .unwrap()
                        .get_mut();
...
pub fn update_source_tree_from_roots(
    source_trees: &mut SourceTrees<Source>,
    room: &Room,
    listener: &Listener,
) {
    for n in 0..source_trees.node_lists.len() {
        for i in 0..source_trees.arenas.len() {
            let parent_node = source_trees.arenas[n]
                .get(source_trees.node_lists[n][i])
                .unwrap()
                .parent();
            match parent_node {
                Some(pn) => {
                    ...
                    current_node.set_pos(calc_ism_position(
                        &parent_position,
                        room,
                        &current_node.get_reflector(),
                    ));
                    current_node
                        .set_dist(calc_distance(&current_node.get_pos(), &listener.get_pos()));
                    current_node.set_remaining_dist(current_node.get_dist() - parent_dist);
                    let lst_src_orientation =
                        calc_lst_src_orientation(&listener, current_node.get_pos());
                    let src_lst_orientation =
                        calc_src_lst_orientation(current_node, current_node.get_pos());
...
fn test_ism_tree_creation() {
    ...
    for i in src_tree.node_lists[0].iter().enumerate() {
        println!(
            "Nr.: {}, {:?}",
            i.0,
            src_tree.arenas[0].get(*i.1).unwrap().get().position
        );
    }
    for i in src_tree.node_lists[1].iter().enumerate() {
        println!(
            "Nr.: {}, {:?}",
            i.0,
            src_tree.arenas[1].get(*i.1).unwrap().get().position
        );
    }
}
#[test]
